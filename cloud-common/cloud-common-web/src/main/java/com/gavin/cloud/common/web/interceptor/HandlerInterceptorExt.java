 ...
 */
public interface HandlerInterceptorExt extends HandlerInterceptor {
     ...
     */
    static boolean isInvokeInternal(HttpServletRequest request) {
        return StringUtils.isBlank(request.getHeader(X_FORWARDED_PREFIX_HEADER));
    }

    static boolean match(String path, List<String> patterns) {
    ...
    default boolean doPreHandle(HttpServletRequest request, HttpServletResponse response, HandlerMethod handlerMethod) {
        return true;
    }
